...
%                 
                ...
                windSpeed = zeros(numDays, numHoursInADay);
                windGust = zeros(numDays, numHoursInADay);
                windBearing = zeros(numDays, numHoursInADay);
                allHours = zeros(numDays, numHoursInADay);
                ...
                for i=1:numDays
                    for j = 1:numHoursInADay       
                        ...
                        windSpeed(i, j) = str2double(out{i,j}.windSpeed);
                        windGust(i, j) = str2double(out{i,j}.windGust);
                        windBearing(i, j) = str2double(out{i,j}.windBearing);
...
%                 figHandle.WindowState = 'maximized';
                ...
                xOffset = 15/60 * (allHours(1, 2) - allHours(1, 1));
                
                
                hourToLength = allHours(1, 2) - allHours(1, 1);
                ...
                switch label
                    ...
                    case '&Pressure'
                        itemtext = text(ax, posTX, posTY, 'Pressure', ...
                                'Clipping', 'on', ...
                                'HorizontalAlignment', 'right', ...
                                'FontWeight', 'bold', ...
                                'FontSize', 8, 'Color', 'black', ...
                    ...
                    case 'Wind Speed'
                        itemtext = text(ax, posTX, posTY, 'Wind Speed', ...
                                'Clipping', 'on', ...
                                'HorizontalAlignment', 'right', ...
                                'FontWeight', 'bold', ...
                                'FontSize', 8, 'Color', 'black', ...
                    ...
                    case 'Wind Gust'
                        itemtext = text(ax, posTX, posTY, 'Wind Gust', ...
                                'Clipping', 'on', ...
                                'HorizontalAlignment', 'right', ...
                                'FontWeight', 'bold', ...
                                'FontSize', 8, 'Color', 'black', ...
                    ...
                    case 'Show ALL'
                        ...
                        itemtext1 = text(ax, posTX, double(ydataMax), 'Wind Gust', ...
                                'Clipping', 'on', ...
                                'HorizontalAlignment', 'right', ...
                                'FontWeight', 'bold', ...
                                'FontSize', 8, 'Color', 'black', ...
                       ...
                       itemtext2 = text(ax, posTX, double(ydataMax+textOffset*2), 'Wind Bearing', ...
                                'Clipping', 'on', ...
                                'HorizontalAlignment', 'right', ...
                                'FontWeight', 'bold', ...
                                'FontSize', 8, 'Color', 'black', ...
                       ...
                       itemtext3 = text(ax, posTX, double(ydataMax+textOffset*4), 'Wind Speed', ...
                                'Clipping', 'on', ...
                                'HorizontalAlignment', 'right', ...
                                'FontWeight', 'bold', ...
                                'FontSize', 8, 'Color', 'black', ...
                       ...
                       itemtext5 = text(ax, posTX, double(ydataMax+textOffset*8), 'Pressure', ...
                                'Clipping', 'on', ...
                                'HorizontalAlignment', 'right', ...
                                'FontWeight', 'bold', ...
                                'FontSize', 8, 'Color', 'black', ...
...
%                 end
                ...
                for i=1:numDays
                    for j = 1:numHoursInADay  
                        if le(allHours(i,j), (xMatlabTime(2))) && ge(allHours(i,j), (xMatlabTime(1)))
                            ...
                            presValues = append(num2str(pressure(j)));
                            tempValues = append(num2str(round(temperature(j),1)));
                            humValues = append(num2str(humidity(j)*100));
                            speedValues = append(num2str(round(windSpeed(j), 1)));
                            gustValues = append(num2str(round(windGust(j), 1)));
                            bearingValues = append(num2str(windBearing(j))); 


                            switch label
                        
                                case '&Pressure'
                                    presTxt = text(ax, allHours(i, j)-xOffset, ...
                                                         double(ydataMax+textOffset), presValues, ...
                                                        'Clipping', 'on', 'Tag', 'pressure', 'FontSize', 8, 'Color', 'black');
                                ...
                                case '&Temperature'
                                    % Temperature  strings to be inserted in the axes
                                     tempTxt = text(ax, allHours(i, j)-xOffset, ...
                                                        ...
                                                        'Clipping', 'on', 'Tag', 'temperature', 'FontSize', 8, 'Color', 'black');
                                ...
                                case '&Humidity'
                                    humidTxt = text(ax, allHours(i, j)-xOffset, ...
                                                         ...
                                                         'Clipping', 'on', 'Tag', 'humidity', 'FontSize', 8, 'Color', 'black');
                                    if ~isequal(rem(24*(numDays-1)+j,numDays),0)
                                        set(humidTxt, 'Visible', 'off');                          
                                    end    

                                case 'Wind Speed'
                                    speedTxt = text(ax, allHours(i, j)-xOffset, ...
                                                         double(ydataMax+textOffset), speedValues, ...
                                                         'Clipping', 'on', 'Tag', 'windspeed', 'FontSize', 8, 'Color', 'black');
                                    if ~isequal(rem(24*(numDays-1)+j,numDays),0)
                                        set(speedTxt, 'Visible', 'off');                          
                                    end
                                                     
                                case 'Wind Gust'
                                    gustTxt = text(ax, allHours(i, j)-xOffset, ...
                                                         double(ydataMax+textOffset), gustValues, ...
                                                         'Clipping', 'on', 'Tag', 'windgust', 'FontSize', 8, 'Color', 'black');
                                    if ~isequal(rem(24*(numDays-1)+j,numDays),0)
...
%                                     bearingTxt = text(ax, allHours(i, j)-xOffset, ...
%                                                          double(ydataMax+textOffset), bearingValues, ...
%                                                          'Clipping', 'on', 'Tag', 'windbearing', 'FontSize', 8, 'Color', 'black');
%                                     if ~isequal(rem(24*(numDays-1)+j,numDays),0)
%                                         set(bearingTxt, 'Visible', 'off');                          
%                                     end
% %                                     [xdir, ydir] = pol2cart(pi/2-str2num(bearingValues)*pi/180, xOffset);
%                                     quint = quiver(ax,(allHours(i,j)-xOffset), (double(ydataMax+textOffset)), xdir, ydir,'LineWidth',2, 'Tag', 'windbearing');
...
%                                     axis normal
                                    ...
                                    triPlot((allHours(i,j)-xOffset), (double(ydataMax+textOffset)), str2num(bearingValues), str2num(bearingValues), yOffset*4, marker_scale)
                                    
                                  
                                case '&Weather Info'

                                    % Create a surface with the images of the weather conditions
                                    s = surface(ax, [allHours(i, j)-xOffset, allHours(i, j)+xOffset; 
                                                 allHours(i, j)-xOffset, allHours(i, j)+xOffset],...
                                    ...
                                    elseif lt(allHours(i,j), xMatlabTime(2)) && lt( xMatlabTime(2) - allHours(i,j), (allHours(1,2) - allHours(1,1)) )
                                ...
                                case 'Show ALL'
                                    ...
                                    % 'Wind Gust'
                                    gustTxt = text(ax, allHours(i, j)-xOffset, ...
                                                 double(ydataMax), gustValues, ...
                                                 'Clipping', 'on', 'FontSize', 8, ...
                                                 'Color', 'black', 'Tag', 'windgust');
                                    if ~isequal(rem(24*(numDays-1)+j,numDays),0)
                                        set(gustTxt, 'Visible', 'off');                          
                                    end
                                    ...
                                    % 'Wind Bearing'
%                                     bearingTxt = text(ax, allHours(i, j)-xOffset, ...
%                                                  double(ydataMax+textOffset*2), bearingValues, ...
%                                                  'Clipping', 'on', 'FontSize', 8, ...
%                                                  'Color', 'black', 'Tag', 'windbearing');
%                                     if ~isequal(rem(24*(numDays-1)+j,numDays),0)
%                                         set(bearingTxt, 'Visible', 'off');                          
%                                     end
                                    ...
                                    marker_scale = max_size / max(modulus);
                                    gca;
                                    triPlot((allHours(i,j)-xOffset), (double(ydataMax+textOffset*2)), str2num(bearingValues), str2num(bearingValues), yOffset*4, marker_scale);
                                    
                                    % 'Wind Speed'
                                    speedTxt = text(ax, allHours(i, j)-xOffset, ...
                                                 double(ydataMax+textOffset*4), speedValues, ...
                                                 'Clipping', 'on', 'FontSize', 8, ...
                                                 'Color', 'black', 'Tag', 'windspeed');
                                    if ~isequal(rem(24*(numDays-1)+j,numDays),0)
                                        set(speedTxt, 'Visible', 'off');                          
                                    end
                ...
                if lt(numHoursInFigure,30)
                    set(findall(ax, 'Type', 'Patch'), 'Visible', 'off');
                    set(findall(ax, 'Type', 'Surface'), 'Visible', 'on');

                elseif gt(numHoursInFigure,30)
                    set(findall(ax, 'Type', 'Patch'), 'Visible', 'on');
                    set(findall(ax, 'Type', 'Surface'), 'Visible', 'off');
                end
    ...
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%% INVISIBLE
                ...
                if ~isempty(getWeatherObj)
                    ...
                    if isequal(legendExist,0)
                        ...
                        lgd = legend(ax2, patchObj, legendIcons, 'Location', 'southwest', 'Tag', 'patchlgd'); 
