...
classdef Meteo_Data < handle
    ...
    methods
        ...
        function [req_mds, id_mds] = get(md_list, marker_name)
            ...
            %
            % SEE ALSO
            %   md_list.get(marker_name)
            req_mds = [];
            id_mds = [];
            for r = 1 : size(md_list,2)
                mds = md_list(r);
                if not(mds.isEmpty)
                    if strcmpi(mds(1).getMarkerName, marker_name)
                        id_mds = [id_mds r]; 
